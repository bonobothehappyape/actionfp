...
import eu.scram.actionfp.config.KafkaProperties;
...
class ActionfpKafkaResourceIT {
    ...
    void setup() {
        ...
        ActionfpKafkaResource kafkaResource = new ActionfpKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/actionfp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/actionfp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
